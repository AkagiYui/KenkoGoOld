...
class GocqProcess:
    ...
    def _read_output(self):
        while self.process.poll() is None:
            ...
            # 删除颜色标签
            match_result = re.match('\x1b\[\d+m', text_output)
            if not match_result:
                continue
            text_output = re.sub('\x1b\[\d+m', '', text_output).strip()
            ...
            if text_output.startswith('账号密码未配置, 将使用二维码登录'):
